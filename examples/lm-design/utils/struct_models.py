...
def load_model():
    model_url_path = (
        "https://dl.fbaipublicfiles.com/fair-esm/examples/lm_design/linear_projection_model.pt"
    )
    local_model_path = hydra.utils.to_absolute_path("./linear_projection_model.pt")
    if not os.path.exists(local_model_path):
        logger.info(
            f"Downloading linear projection model from {model_url_path} to {local_model_path}"
        )
    ...
    from fairesm.pretrained import esm2_t33_650M_UR50D
...
class WrapStructModel(WrapStruct):
    def forward(self, seq1h, pos_idx=None):
        ...
        if embed_dim != K:
            seq1h = torch.cat([seq1h, torch.zeros(B, L + 1, embed_dim - K).to(seq1h)], -1)
