...
sys.path.append("../../")
from fairesm.data import Alphabet
...
class Designer:
    ...
    def __init__(self, cfg, target_pdb_path=None, device=None):
        ...
        self.allowed_AA = "".join(
            ...
            if (("suppress_AA" not in self.cfg) or (not AA in self.cfg.suppress_AA))
        )
    ...
    def decode(self, seq_enc, onehot=True):
        ...
        # large slowdown.
        seq_enc = seq_enc.cpu()
        seqs = ["".join([self.vocab.get_tok(c) for c in _seq]) for _seq in seq_enc]
        return seqs
    ...
    def _init_target(self, pdb_path):
        ...
        self.valid_contacts = torch.from_numpy(
            self.target_data["dist"] == self.target_data["dist"]
        ).to(self.device)
    ...
    def init_sequences(self, num_seqs):
        ...
        bt = torch.from_numpy(np.random.choice(AA_indices.cpu().numpy(), size=(B, self.L))).to(
            self.device
        )
    ...
    def calc_total_loss(self, x, mask, LM_w, struct_w, ngram_w, ngram_orders, temp_struct=None):
        """
    ...
    def init_schedulers_from_cfg(self, cfg: DictConfig):
        ...
        def is_sspec(maybe_sspec: Union[SchedulerSpec, Any]):
            infer_from_key = (
                isinstance(maybe_sspec, DictConfig)
